...
export const useNotify = defineStore('notify', () => {
  ...
  function notify (notification: Pick<Notification, 'type' | 'text'> & {timeout?: number}) {
  ...
  function timeoutKiller () {
    ...
    const livingNotification = notifications.value
      .filter(notification => notification.createdAt.getTime() + notification.timeout > now)
